...
async def readiness_check():
    ...
    if provider not in {"claude", "anthropic", "openai", "deepseek"}:
        config_issues.append(f"Unsupported LLM provider: {provider}")
    elif not settings.active_llm_api_key:
        config_issues.append(f"No API key configured for provider: {provider}")
