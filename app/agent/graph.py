...
def _create_llm():
    ...
    if provider in {"claude", "anthropic"}:
        ...
        logger.info("Initializing LLM provider=anthropic model=%s", model)
    ...
    if provider == "openai":
        ...
        logger.info("Initializing LLM provider=openai model=%s", model)
    ...
    if provider == "deepseek":
        ...
        if settings.llm_thinking:
            raise RuntimeError(
                "LLM_THINKING=true is not supported by the current "
                "langchain-openai tool loop. Set LLM_THINKING=false."
            )

        from langchain_openai import ChatOpenAI

        model = configured_model or "deepseek-v4-flash"
        base_url = (settings.llm_base_url or "https://api.deepseek.com").strip().rstrip("/")
        if not base_url.startswith(("http://", "https://")):
            raise RuntimeError(
                "LLM_BASE_URL must be a plain http(s) URL, for example "
                "https://api.deepseek.com."
            )

        logger.info(
            "Initializing LLM provider=deepseek model=%s thinking=disabled",
            model,
        )
        return ChatOpenAI(
            model=model,
            ...
            extra_body={
                ...
                # DeepSeek expects max_tokens. ChatOpenAI 0.3.0 otherwise
                # rewrites this setting as max_completion_tokens.
                "max_tokens": settings.llm_max_tokens,
            },
        )

    raise RuntimeError(
        "Unsupported LLM_PROVIDER=%r. Use deepseek, openai, or anthropic."
        % provider
    )
