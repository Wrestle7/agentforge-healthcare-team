...
团队成员请先阅读 [协作与本地启动说明](CONTRIBUTING.md)。团队仓库的默认分支为 `main`，以不含本地运行数据的代码快照开始；原项目来源保留在文档中。
...
User (Streamlit Chat UI)
   ...
   FastAPI backend (:8000)
        |
   LangGraph Agent (DeepSeek / OpenAI / Claude)
        |
...
- **LLM:** Configurable DeepSeek, OpenAI, or Anthropic provider
...
- API key for DeepSeek, OpenAI, or Anthropic
